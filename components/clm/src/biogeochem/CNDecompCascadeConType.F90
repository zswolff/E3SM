module CNDecompCascadeConType
  ...
  type, public :: decomp_cascade_type
     ...
     character(len=8) , pointer :: cascade_step_name(:)               => null()! name of transition
     integer          , pointer :: cascade_donor_pool(:)              => null()! which pool is C taken from for a given decomposition step
     integer          , pointer :: cascade_receiver_pool(:)           => null()! which pool is C added to for a given decomposition step
     ...
     real(r8)         , pointer  :: spinup_factor(:)                  => null()! factor by which to scale AD and relevant processes
     real(r8)         , pointer  :: decomp_k_pools(:)                 => null()! Kd in 1/sec for pool
...
contains
  ...
  subroutine init_decomp_cascade_constants()
    ...
    decomp_cascade_con%decomp_k_pools(0:ndecomp_pools)                 = spval
